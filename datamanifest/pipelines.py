...
#
# The implementation now lives in the Layer 0 substrate
# (:mod:`datamanifest.store.materialize`) so the cache layer can reuse it without
# importing the fetch layer. Re-exported here under the historical names
# (``materialize`` / ``is_complete`` / ``_remove_path``) so existing call sites
# and ``datamanifest.pipelines`` importers keep working unchanged.

from .store.materialize import (  # noqa: E402
    _acquire_lock,
    _pid_alive,
    _read_lock_pid,
    is_complete,
...
def download_dataset(db, dataset, extract=None, overwrite: bool = False):
    ...
    # Lazy / on-the-fly access: never download; the uri is opened in place by the
    # loader. Nothing local to verify or record.
    if dataset.lazy_access:
        logger.info("Lazy access (not downloaded): %s", dataset.uri)
