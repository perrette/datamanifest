...
| `add URI\|DOI [--name N] [--pick GLOB] [--no-download] [--extract] [--on-the-fly]` | Register and (by default) download a dataset. A **Zenodo** DOI / record URL expands to one dataset per file (declare-only; `--pick GLOB` selects a subset, `--name` is the name prefix). `--on-the-fly` registers a remote URI (`s3://`, `gs://`, …) for lazy access instead of downloading — it sets `skip_download` and a built-in fsspec loader, so `load()` opens it in place |
